"""Test low action_scale 0.28-0.34 for straight + turn stability."""
...
print("act  vx  vyaw  mean_bodyvx  p90   final_z  t_fall  disp/s", flush=True)
for act, vx, vyaw, dur in cases:
    ...
    print(f"{act:.2f} {vx:.2f} {vyaw:+.2f}  {s.mean():+.3f}  {np.percentile(s,90):+.3f}  {data.qpos[2]:.3f}  {tf}  {disp/max(el,1e-6):.3f}", flush=True)
